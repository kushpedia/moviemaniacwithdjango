...
def register(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            profile.profile_picture = 'userprofile/default.jpg'
            profile.save()
...
def login(request):
    if request.method == "POST":
        ...
        if user is not None:
                    # check if there is a cart item in the cart
            try:
                ...
                if is_cart_item_exists:
                    ...
                    # get cart items by the user to access his product variations
                    ...
                    for pr in product_variation:
                        if pr in ex_var_list:
                            ...
                            cart_item = CartItem.objects.filter(cart=cart)# set user on the cart item
                            for item in cart_item:
                                item.user = user
                                item.save() 
...
def logout(request):
    auth.logout(request)
    messages.error(request, 'Logged Out Successful')
